...
clear
NUMBER_OF_CLIENTS=$(grep -c -E "^### " "/etc/mon/xray/uservless.txt")
	...
	echo -e "==============================="
	grep -E "^### " "/etc/mon/xray/uservless.txt" | cut -d ' ' -f 2-3 | nl -s ') '
...
user=$(grep -E "^### " "/etc/mon/xray/uservless.txt" | cut -d ' ' -f 2 | sed -n "${CLIENT_NUMBER}"p)
exp=$(grep -E "^### " "/etc/mon/xray/uservless.txt" | cut -d ' ' -f 3 | sed -n "${CLIENT_NUMBER}"p)
...
sed -i "s/### $user $exp/### $user $exp4/g" /etc/mon/xray/uservless.txt
